...
def ban(update: Update, context: CallbackContext) -> str:
    ...
    if is_user_ban_protected(chat, user_id, member) and user not in DEV_USERS:
        if user_id == OWNER_ID:
            message.reply_text("Trying to put me against My Owner huh why are you so gae?")
        elif user_id in DEV_USERS:
            message.reply_text("I can't act against our Co-Owners.")
        elif user_id in DRAGONS:
            message.reply_text(
                "Fighting this ADMIN here will put civilian lives at risk."
            )
        elif user_id in DEMONS:
            message.reply_text(
                "Bring an order from Nexus to fight this MOD."
            )
        elif user_id in TIGERS:
            message.reply_text(
                "Bring an order from Nexus to fight this Trust Worthy Mem."
            )
        elif user_id in WOLVES:
            message.reply_text("Good Guys abilities make them ban immune!")
        else:
...
*Admins only:*
 ...
 • `/kick <userhandle>`*:* kicks  user out of the group, (via handle, or reply)
"""
...
PUNCHME_HANDLER = DisableAbleCommandHandler("kickme", kickme, filters=Filters.group)
